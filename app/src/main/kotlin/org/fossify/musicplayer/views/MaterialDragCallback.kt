...
/**
 * A highly customized [ItemTouchHelper.Callback] that enables the lift animation and swipe-to-remove
 * scrim of the queue. Requires a [ViewHolder] implementation in order to function.
 */
abstract class MaterialDragCallback : ItemTouchHelper.Callback() {
    ...
    enum class CausedBy { CLICK, RELEASE }

    private val itemCornerSpring = Spatial.DEFAULT
    private val itemAlphaSpring = Effect.DEFAULT
    private val itemTranslateZSpring = Spatial.DEFAULT
    private val itemAnims = mutableMapOf<View, AnimBundle>()

    final override fun getMovementFlags(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) =
    ...
    final override fun onChildDraw(
        ...
        // only done once when the item is initially picked up.
        if (isCurrentlyActive && actionState == ItemTouchHelper.ACTION_STATE_DRAG) {
            ...
            val elevation = recyclerView.context.resources.getDimension(MR.dimen.m3_sys_elevation_level4)
            val cornerSize = recyclerView.context.resources.getDimension(R.dimen.corner_radius_large)
            ...
            if (itemAnim?.causedBy != CausedBy.CLICK) {
                ...
                itemAnims[holder.root] = AnimBundle(
                    ...
                    translateZAnim = itemTranslateZSpring.translateZ(holder.root, elevation),
        ...
        // We show a background with a delete icon behind the item each time one is swiped away.
        // The background view would otherwise lag behind the body view, so it is kept invisible
        // whenever the item is not being swiped.
        ...
        val cornerSize = recyclerView.context.resources.getDimension(R.dimen.corner_radius_large)
        val threshold = recyclerView.context.resources.getDimensionPixelSize(R.dimen.size_touchable_large)
    ...
    final override fun clearView(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        ...
        if (holder.root.translationZ != 0f) {
            ...
            if (itemAnim?.causedBy != CausedBy.RELEASE) {
                ...
                itemAnims[holder.root] = AnimBundle(
                    ...
                    translateZAnim = itemTranslateZSpring.translateZ(holder.root, 0f),
        ...
        // Reset translations. We do not call the default implementation, so we must do this
        // ourselves.
