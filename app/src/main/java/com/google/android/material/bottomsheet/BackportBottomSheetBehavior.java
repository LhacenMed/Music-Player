...
public class BackportBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V>
  ...
  // MODIFICATION: Recolour the sheet from the app's palette. The background is owned by
  // materialShapeDrawable and re-applied on every layout, and the colour is only known at runtime,
  // so it cannot come from the sheet's style.
  public void setSheetFillColor(@NonNull ColorStateList fillColor) {
    if (materialShapeDrawable != null) {
